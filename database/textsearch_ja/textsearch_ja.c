...
#define NUM_CSV			9
#define MECAB_TYPE      0
#define MECAB_SUBTYPE1  1
#define MECAB_SUBTYPE2  2
#define MECAB_SUBTYPE3  3
#define MECAB_CONJTYPE  4
#define MECAB_CONJUGATION  5
...
PG_FUNCTION_INFO_V1(ts_ja_lexize_with_types);
...
extern Datum PGDLLEXPORT ts_ja_lexize_with_types(PG_FUNCTION_ARGS);
...
ts_ja_gettoken(PG_FUNCTION_ARGS)
{
	...
	/* 無視する品詞は blank 扱い. */
	if (0 /* ignore(node) */) // EDIT: We want everything to be searched.
		lextype = SPACE;
	else
		lextype = WORD_T;
...
char *concat_with_dot(const char *s, const char *t) {
	char *r;
	...
	{
	case PG_UTF8:
		r = palloc(slen + tlen + 4);
		dot = (char[]){0xe3, 0x83, 0xbb};
		memcpy(r, s, slen);
		memcpy(r + slen, dot, 3);
		memcpy(r + slen + 3, t, tlen);
		r[slen + tlen + 3] = '\0';
		break;
	case PG_EUC_JP:
	case PG_EUC_JIS_2004:
		r = palloc(slen + tlen + 3);
		dot = (char[]){0xa1, 0xa6};
		memcpy(r, s, slen);
		memcpy(r + slen, dot, 2);
		memcpy(r + slen + 2, t, tlen);
		r[slen + tlen + 2] = '\0';
		break;
	default: /* not supported */
		r = palloc(slen + tlen + 1);
		memcpy(r, s, slen);
...
// EDIT: Include token types to allow searching by part of speech etc.

Datum
ts_ja_lexize_with_types(PG_FUNCTION_ARGS)
{
	const char *t = (char *) PG_GETARG_POINTER(1);
	int			tlen = PG_GETARG_INT32(2);
	TSLexeme   *res = palloc0(sizeof(TSLexeme) * (2 + 1));

	char *mecab_type;
	// char *mecab_conjugation;
	// char *mecab_conjtype;
	...
	if (current_node) {
		...
		if (feature(current_node, MECAB_TYPE, &t, &tlen)) {
			mecab_type = lexize(t, tlen);
			res[1].lexeme = add_hashtag(mecab_type);
			if (feature(current_node, MECAB_SUBTYPE1, &t, &tlen)) {
				res[1].lexeme = concat_with_dot(res[1].lexeme, lexize(t, tlen));
			}
			if (feature(current_node, MECAB_SUBTYPE2, &t, &tlen)) {
				res[1].lexeme = concat_with_dot(res[1].lexeme, lexize(t, tlen));
			}
			if (feature(current_node, MECAB_SUBTYPE3, &t, &tlen)) {
				res[1].lexeme = concat_with_dot(res[1].lexeme, lexize(t, tlen));
			}
			// if (feature(current_node, MECAB_CONJUGATION, &t, &tlen)) {
			// 	mecab_conjugation = lexize(t, tlen);
			// 	res[2].lexeme = add_hashtag(mecab_type);
			// 	res[2].lexeme = concat_with_dot(res[2].lexeme, mecab_conjugation);
			// 	res[1].lexeme = concat_with_dot(res[1].lexeme, mecab_conjugation);
			// 	if (feature(current_node, MECAB_CONJTYPE, &t, &tlen)) {
			// 		mecab_conjtype = lexize(t, tlen);
			// 		res[3].lexeme = add_hashtag(mecab_type);
			// 		res[3].lexeme = concat_with_dot(res[3].lexeme, mecab_conjtype);
			// 		res[3].lexeme = concat_with_dot(res[3].lexeme, mecab_conjugation);
			// 		res[1].lexeme = concat_with_dot(res[2].lexeme, mecab_conjtype);
			// 		res[1].lexeme = concat_with_dot(res[1].lexeme, mecab_conjtype);
			// 	}
			// }
		}
	} else {
